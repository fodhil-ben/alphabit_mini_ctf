...
app.post('/', (req, res) => {
    if (req.headers['user-agent'] === 'l33t_br0w$3r') {
        if (req.headers['authorization'] === 'authoride') {
            if (req.headers['content-language'] === 'arabic') {
                if (req.headers['location'] === 'alphabit') {
                    ...
                    res.send('to get the flag you must be in alphabit')
                }
            } else {
                res.send('You must talk arabic')
            }
        } else {
            res.send('You must be authoride to get the flag')
        }
    } else {
